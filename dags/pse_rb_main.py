...
def fetch_entity_data(entity: str, execution_date=None) -> dict:
    ...
    try:
        ...
        # Log sukcesu fetch
        #pg_hook.run(log_sql, parameters=(entity, date_str, "Data fetched successfully", "success"))
        ...
        error_msg = str(e)
        logger.error(f"Error fetching data for {entity} on {date_str}: {error_msg}")

        # Log błędu
        #pg_hook.run(log_sql, parameters=(entity, date_str, error_msg, "fail"))
...
def log_etl_result(staging_info: dict, execution_date=None):
    ...
    if staging_info["skipped"]:
        ...
        try:
            # Sprawdź, czy dane trafiły do produkcyjnej tabeli
            prod_table = ENTITY_CONFIG[entity]["production_table"]
            sql = f"""
                SELECT COUNT(*) FROM airflow_data."{prod_table}" WHERE business_date = %s;
            """
            count = pg_hook.get_first(sql, parameters=(date_str,))[0]
            if count > 0:
                message = f"Records loaded into production table"
                result = "success"
            else:
                message = "No data in production table after merge"
                result = "fail"
        except Exception as e:
            message = f"Error checking production table: {str(e)}"
            result = "fail"
    ...
    log_sql = """
        ...
        VALUES (%s, %s, %s, %s);
    """
    pg_hook.run(log_sql, parameters=(entity, date_str, message, result))
...
def pse_etl_main():
    ...
    for entity in ENTITY_CONFIG:
        ...
        data.set_upstream([tables, log_table])
        ...
        log_result = log_etl_result(staged)
