...
# 🔧 1. entsoe_dag_config.py – konfiguracja (minimalna wersja)
...
# 🧱 1. create_initial_tables() – tworzenie tabeli PostgreSQL
...
def create_initial_tables(db_conn_id: str, raw_xml_table: str) -> dict:
    ...
    #logger.info(f"Created or confirmed table airflow_data.\"{raw_xml_table}\".")
    return {"raw_xml_table": raw_xml_table}


# 🧠 2. Parametry dla każdego kraju + roku)
...
def generate_run_parameters(**context) -> List[Dict[str, str]]:
    ...
    # Rok, dla którego działa aktualna instancja DAG-a
    year = context['data_interval_start'].year

    return _generate_run_parameters_logic(year)


#🔄 3. extract_from_api() – pobieranie danych z ENTSO-E
...
#💾 4. extract_from_api() – pobieranie danych z ENTSO-E
...
def store_raw_xml(extracted_data: Dict[str, Any], db_conn_id: str, table_name: str) -> int:
    if not extracted_data or 'xml_content' not in extracted_data:
        #logger.warning(f"Brak danych do zapisu. Params: {extracted_data.get('request_params')}")
        return -1
    ...
    try:
        ...
        #logger.info(f"Zapisano XML z ID {inserted_id} ({extracted_data.get('country_name')})")
        return inserted_id
    except Exception as e:
...
#🔄 5. parse_generation_units)metadata() – pobieranie danych z ENTSO-E
...
def parse_generation_units_metadata(extracted_data: Dict[str, Any]) -> pd.DataFrame:
    ...
    implementation_date = extracted_data['period_start']  # np. '2023-01-01'
    ...
    for ts in root.findall('ns:TimeSeries', ns):
        ...
        #year_api = int((impl_date or implementation_date).split("-")[0])
        year_api = int(task_param['entsoe_api_params']["Implementation_DateAndOrTime"].split("-")[0])
        ...
        if gu_elements:
            ...
            # fallback: brak jednostek – agregowany wpis z plant-level
    ...
    # Jeśli chcesz mieć 100% gwarancji, że wszystkie kolumny są obecne nawet dla pustych danych
    expected_columns = [
...
def merge_data_to_production(staging_dict: Dict[str, Any], db_conn_id: str) -> str:
    ...
    production_table_name = "generation_units"
    pg_hook = PostgresHook(postgres_conn_id=db_conn_id)
